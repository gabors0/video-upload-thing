document.addEventListener('DOMContentLoaded', () => {
    ...
    dropZone.addEventListener('dragleave', (e) => {
        ...
        dropZone.classList.add('bg-neutral-900');
    });
        ...
        uploadForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    successMessage.classList.add('hidden');
                    errorMessage.querySelector('.message-text').textContent = result.message || 'Unknown error';
                }
                ...
                errorMessage.classList.remove('hidden');
                successMessage.classList.add('hidden');
                errorMessage.querySelector('.message-text').textContent = error.message || 'Network error';
            }
        });
    }
    ...
    async function loadVideos() {
        try {
            console.log('Fetching videos from /videos');
            ...
            if (data.success && Array.isArray(data.videos)) {
                if (data.videos.length > 0) {
                    ...
                    videoListDiv.innerHTML = `
                        <div class="bg-neutral-800 p-4 rounded-lg text-center text-neutral-400">
    ...
    window.deleteVideo = async function(filename) {
        ...
        try {
            ...
            errorMessage.classList.remove('hidden');
            successMessage.classList.add('hidden');
            errorMessage.querySelector('.message-text').textContent = `Error deleting ${filename}: ${error.message}`;
        }
    };
    ...
    if (checkedRadio) {
        ...
        videoListDiv.classList.remove("grid-cols-1", "grid-cols-2", "grid-cols-3", "grid-cols-4");
